...
const MASTER_STORAGE_KEY = 'cosmic_compass_master_v4';
const DEFAULT_FALLBACK_KEY = "AIzaSyDrFjYv2c322zzCMsgpVttjUz9lWDrBoUg"; // Fallback only, likely restricted
...
const DEFAULT_STATE: AppState = {
  ...
  enableGoogleSearch: false, // Default to false to prevent 403 on restricted keys
  chatHistory: [],
  visuals: {},
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const recoverData = async () => {
      try {
        ...
        console.error("Cloud Access Failure (Offline Mode Active):", e);
      } finally {
  ...
  const getEffectiveApiKey = () => customApiKey.trim() || DEFAULT_FALLBACK_KEY;
  ...
  const handleSendMessage = async (message: string) => {
    ...
    if (!session) {
      ...
      try {
        ...
        handleApiError(err, setChatError);
  ...
  const handleApiError = (err: any, setErrorCallback: (msg: string) => void) => {
    console.error(err);
    if (err.toString().includes('403') || err.message?.includes('403')) {
      setErrorCallback("Access Forbidden (403). Check API Key in Settings or disable Search.");
      setShowSettings(true);
    } else {
      setErrorCallback(err.message || "Unknown error occurred");
    }
  };
          ...
          <div className="glass bg-[#0f172a] rounded-3xl p-8 max-w-md w-full border border-white/20 shadow-2xl relative">
             ...
             <div className="space-y-6">
                <div>
                   ...
                   />
                   <p className="text-[10px] text-gray-500 mt-2">Required for 'Pro' models and Search.</p>
                </div>
