...
/**
 * Compresses an image data URI to stay within token limits.
 */
const compressImage = async (dataUri, maxWidth = 800) => {
...
async function getCombinedReading(
  ...
  try {
    ...
    if (message.includes("429") || message.includes("RESOURCE_EXHAUSTED") || errString.includes("429")) {
      let advice = "Wait 60 seconds and try again.";
      if (errString.includes("quota")) {
        advice = "You've likely hit your DAILY limit for the Free Tier. Try again tomorrow or use a different API Key.";
      }
      throw new Error(`Quota Exhausted (429): ${advice} \n\nTIP: Disable 'Google Search' and remove large images to save quota.`);
    }
    throw error;
  }
}
...
const DEFAULT_STATE = {
  ...
  frontierParams: {
    ...
    environmentType: 'Urban (High Connectivity)',
    biologicalAge: '30'
  },
  chatHistory: [],
  visuals: {},
...
const App = () => {
  ...
  const [apiKey, setApiKey] = useState(() => {
    return localStorage.getItem('cosmic_api_key') || getSafeEnvApiKey();
  });
  ...
  const handleSendMessage = async (message) => {
    ...
    try {
      ...
      if (errStr.includes("Service Blocked") || errStr.includes("403")) {
        ...
        errorMessage = "Quota Exhausted (429): Please wait 60 seconds before asking again, or disable Google Search in Settings.";
      } else if (err.message) {
  ...
  const handleFileUpload = (type) => (e) => {
    ...
    if (file) {
      ...
      reader.onloadend = async () => {
        ...
        // Compress image before saving to state to save tokens and Firebase space
                        ...
                        <li key={idx} className="flex items-start flex-col">
                          ...
                          {source.maps && (
                            <div className="flex items-start flex-col mb-1">
                              ...
                              {source.maps.placeAnswerSources?.reviewSnippets?.map((snippet, sIdx) => (
